...
Clone the git repositories for mtcoin and gitian and then checkout the mtcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the mtcoin repository
...
To build Mtcoin (for Linux, OSX and Windows) just follow the steps under 'perform
...
For example:
```bash
URL=https://github.com/konez2k/mtcoin.git
...
./bin/gbuild --commit mtcoin=${COMMIT} --url mtcoin=${URL} ../mtcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mtcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mtcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/mtcoin-build.assert
```
...
[mtcoin/gitian.sigs](https://github.com/mtcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
